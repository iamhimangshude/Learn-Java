...
public class Main {
    public static void main() {
        ...
        String[] fruit = { "apple", "banana", "pea", "BANANA" };
        ...
        for (var stu : students) {
            int val = tim.compareTo(stu);
            System.out.printf("%s %s %s: compareTo=%d%n", tim, val == 0 ? "==" : (val < 0) ? "<" : ">", stu, val);
        }
...
class StudentGPAComparator implements Comparator<Student> {
    ...
    public int compare(Student o1, Student o2) {
        return (o1 + o1.name).compareTo(o2.gpa + o2.name);
    }
}
...
class Student implements Comparable<Student> {
    ...
    // This is explicitly typecasted to Student class and is not generic
    // @Override
